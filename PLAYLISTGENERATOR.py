import streamlit as st
from huggingface_hub import InferenceClient
import requests
import random
import urllib.parse

# --- КОНФИГУРАЦИЯ СИСТЕМЫ ---
DEFAULT_HF_TOKEN = "" 
MODEL_REPO_ID = "Qwen/Qwen2.5-72B-Instruct"

# --- BACKEND: СЕРВИС ПОЛУЧЕНИЯ ДАННЫХ ---
def fetch_artist_metadata(artist_name: str) -> dict | None:
    """
    Получает метаданные артиста, топ-треки и список похожих исполнителей через API Deezer.
    Реализует стохастическую (случайную) выборку из пула похожих артистов для обеспечения вариативности результатов.
    """
    try:
        # 1. Получение ID артиста и базовой информации
        search_url = f"https://api.deezer.com/search/artist?q={artist_name}"
        response = requests.get(search_url).json()
        
        if not response.get('data'):
            return None
            
        artist_obj = response['data'][0]
        artist_id = artist_obj['id']
        real_name = artist_obj['name']
        
        # 2. Получение топ-треков исходного исполнителя
        tracks_url = f"https://api.deezer.com/artist/{artist_id}/top?limit=4"
        tracks_data = requests.get(tracks_url).json().get('data', [])
        source_tracks = [t['title'] for t in tracks_data]
        
        # 3. Получение пула похожих исполнителей (Лимит 20 для выборки)
        related_url = f"https://api.deezer.com/artist/{artist_id}/related?limit=20"
        related_data_all = requests.get(related_url).json().get('data', [])
        
        # Выборка подмножества (Random Sampling)
        if len(related_data_all) > 5:
            related_data = random.sample(related_data_all, 5)
        else:
            related_data = related_data_all
        
        similar_artists_info = []
        for rel_artist in related_data:
            # Получение топ-треков для каждого связанного артиста
            r_id = rel_artist['id']
            r_tracks_url = f"https://api.deezer.com/artist/{r_id}/top?limit=2"
            r_tracks_data = requests.get(r_tracks_url).json().get('data', [])
            r_tracks = [t['title'] for t in r_tracks_data]
            
            similar_artists_info.append({
                "name": rel_artist['name'],
                "tracks": r_tracks
            })
            
        return {
            "source_artist": real_name,
            "source_tracks": source_tracks,
            "similar": similar_artists_info
        }
    except Exception as e:
        # Логирование ошибки (в продакшене здесь была бы запись в лог-файл)
        print(f"API Error: {e}")
        return None

# --- КЛАСС ИНТЕЛЛЕКТУАЛЬНОГО АГЕНТА ---
class Agent:
    """
    Представляет автономного агента на базе LLM с заданной ролью и контекстом выполнения.
    """
    def __init__(self, name: str, role: str, client: InferenceClient):
        self.name = name
        self.role = role
        self.client = client

    def execute(self, input_context: str) -> str:
        """Отправляет структурированный промпт к Inference API."""
        messages = [
            {
                "role": "system", 
                "content": f"""
                РОЛЬ: {self.name}
                ЗАДАЧА: {self.role}
                ОГРАНИЧЕНИЯ:
                1. Язык вывода: Русский.
                2. Использовать Markdown разметку.
                3. Строго придерживаться предоставленного контекста данных.
                """
            },
            {
                "role": "user", 
                "content": f"КОНТЕКСТ ДАННЫХ:\n{input_context}"
            }
        ]
        try:
            response = self.client.chat_completion(
                messages=messages, 
                max_tokens=1500, 
                temperature=0.4, # Баланс между точностью и креативностью
                top_p=0.9
            )
            return response.choices[0].message.content
        except Exception as e:
            return f"Ошибка выполнения агента: {e}"

# --- ПОЛЬЗОВАТЕЛЬСКИЙ ИНТЕРФЕЙС (STREAMLIT) ---
def main():
    st.set_page_config(page_title="Интеллектуальная МАС", page_icon="🎵", layout="wide")
    
    # Заголовок приложения
    st.title("🎵 Интеллектуальная Система Генерации Плейлистов")
    st.markdown("### Лабораторная работа: Мультиагентные системы")
    st.markdown("**Выполнили:** Грудницкий и Соловьёв")
    st.markdown("---")

    # Сайдбар конфигурации
    with st.sidebar:
        st.header("Конфигурация Системы")
        hf_token = st.text_input("Токен Hugging Face API", value=DEFAULT_HF_TOKEN, type="password")
        if not hf_token:
            st.warning("Ожидание токена авторизации...")
            return

    # Инициализация клиента
    client = InferenceClient(model=MODEL_REPO_ID, token=hf_token)

    # Блок ввода данных
    col1, col2 = st.columns([3, 1])
    with col1:
        user_query = st.text_input("Ввод Исполнителя / Референс", "Linkin Park")
    with col2:
        st.write("")
        st.write("")
        run_btn = st.button("Инициализировать Агентов", type="primary")

    if run_btn and user_query:
        
        # --- ЭТАП 0: СБОР ДАННЫХ (DATA ACQUISITION) ---
        raw_data = None
        with st.status("Система: Получение метаданных из внешнего API...", expanded=True) as status:
            raw_data = fetch_artist_metadata(user_query)
            if raw_data:
                st.success(f"Целевой объект идентифицирован: {raw_data['source_artist']}. Связанные сущности получены.")
                status.update(label="Сбор данных завершен", state="complete")
            else:
                status.update(label="Ошибка сбора данных", state="error")
                st.error("Исполнитель не найден в базе данных.")
                st.stop()

        # Формирование контекста для агентов
        context_payload = f"ЦЕЛЕВАЯ СУЩНОСТЬ: {raw_data['source_artist']} (Топ-треки: {', '.join(raw_data['source_tracks'])})\n"
        context_payload += "СВЯЗАННЫЕ СУЩНОСТИ:\n"
        for s in raw_data['similar']:
            context_payload += f"- {s['name']} (Треки: {', '.join(s['tracks'])})\n"

        st.divider()

        # --- ЭТАП 1: СЕМАНТИЧЕСКИЙ АНАЛИЗ ---
        st.subheader("🕵️ Агент 1: Семантический Анализ")
        agent1 = Agent(
            name="Агент 1 (Анализатор Сходства)",
            role="Проанализируй предоставленный список связанных исполнителей. Объясни стилистические и жанровые связи между целевым артистом и связанными сущностями.",
            client=client
        )
        with st.chat_message("assistant", avatar="🕵️"):
            with st.spinner("Выполнение семантического анализа..."):
                output_1 = agent1.execute(context_payload)
                st.markdown(output_1)

        # --- ЭТАП 2: АГРЕГАЦИЯ ПЛЕЙЛИСТА ---
        st.subheader("💿 Агент 2: Агрегация Плейлиста")
        agent2 = Agent(
            name="Агент 2 (Композитор Плейлиста)",
            role="""
            Сформируй единый список треков на основе предоставленного контекста.
            Включи треки как целевого исполнителя, так и связанных сущностей.
            Формат вывода: 'Исполнитель - Название Трека'.
            Используй только предоставленные входные данные, не генерируй несуществующие объекты.
            """,
            client=client
        )
        with st.chat_message("assistant", avatar="💿"):
            with st.spinner("Компиляция списка треков..."):
                output_2 = agent2.execute(context_payload)
                st.markdown(output_2)

        # --- ЭТАП 3: КЛАСТЕРИЗАЦИЯ И ФИЛЬТРАЦИЯ ---
        st.subheader("📊 Агент 3: Кластеризация по Настроению")
        agent3 = Agent(
            name="Агент 3 (Классификатор)",
            role="""
            Выполни кластеризацию предоставленного списка треков.
            1. Проанализируй аудио-характеристики (подразумеваемые жанром).
            2. Раздели список на две контрастные категории настроения (например, 'Высокая Энергия' vs 'Мелодичное/Атмосферное').
            3. Выведи отсортированные списки под соответствующими заголовками.
            """,
            client=client
        )
        with st.chat_message("assistant", avatar="📊"):
            with st.spinner("Классификация аудио-признаков..."):
                output_3 = agent3.execute(output_2)
                st.markdown(output_3)

        # --- ЭТАП 4: МЕХАНИЗМ РЕКОМЕНДАЦИЙ ---
        st.subheader("🟢 Агент 4: Рекомендательная Система")
        agent4 = Agent(
            name="Агент 4 (Discovery Engine)",
            role="""
            Задача 1: Симуляция Коллаборативной Фильтрации.
            Предложи 3 НОВЫХ трека от исполнителей, НЕ представленных в предыдущих списках, которые соответствуют выявленным жанрам.
            
            Задача 2: Интеграция с Внешними Сервисами (Spotify).
            Предложи 3 названия популярных плейлистов, соответствующих этому настроению.
            Сгенерируй валидные ссылки на поиск (Search URL).
            
            Формат ссылки: `[Название Плейлиста](https://open.spotify.com/search/ЗАПРОС)`
            (Замени ЗАПРОС на название плейлиста или жанра).
            """,
            client=client
        )
        with st.chat_message("assistant", avatar="🟢"):
            with st.spinner("Генерация рекомендаций и ссылок..."):
                output_4 = agent4.execute(output_3)
                st.success("Альтернативные Миксы и Discovery:")
                st.markdown(output_4)

if __name__ == "__main__":
    main()